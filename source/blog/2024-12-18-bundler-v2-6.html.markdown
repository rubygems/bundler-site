...
* For a single lockfile, you can run `bundle lock --add-checkums`. This will add
  a new `CHECKSUMS` section of the lockfile that Bundler will keep up to date.

* To configure Bundler to always include checksums in new lockfiles: `bundle
...
`BUNDLED WITH` lockfile section. Since Bundler internally has a mechanism to
make sure it switches to the version of itself included in the `BUNDLED WITH`
lockfile section, a lockfile with a `CHECKSUMS` section should always be run by
Bundler >= 2.6.
